...
- Comments explain intent and decisions, not line-by-line mechanics.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc with `@param` and `@returns`.
- Reusable `.astro` components document their `Props` interface as the component API contract.
